...
u1=plot(((D)),'-','LineWidth',2, 'color', c3); 
...
%%%%
ylabel('Infection rate \beta_a(H) & R0(H,V)','FontSize', 12)
xlabel('Day since April 15, 2021', 'FontSize', 12)
b1=legend([a1,a2,a22,a3],'Infection rate \beta_a(t)=\gamma(1-Ef)', ...
    'R0(t)(Model 3)','R0(t) 95% CI', '\beta,R0=1','FontSize', 10,'Location','northwest');
set(b1,'Box','off')
axis([0, length(H), 0, 6]);
box on
...
x1 = linspace(0,50);
y1 = polyval(pv,x1);
b1=legend([a1,a2,a3],'Infection rate \beta_a=\gamma(1-Ef)', 'R0 (Model 3)', '\beta,R0=1','FontSize', 10,'Location','northeast');
set(b1,'Box','off')
axis([0, 50, 0, 4]);
box on
...
b1=legend('Vp=0%','Vp=10%','Vp=20%','Vp=30%','Vp=40%','Vp=50%','Vp=60%','Vp=70%','Vp=80%','Vp=90%');%,'Ef2') 
set(b1,'Box','off')
axis([0, 100, 0, 2]);
box on
end
...
yl =[0*(xl)+1];
line(xl,yl,'Color','black','LineStyle','--');
b1=legend('Vp=0%','Vp=10%','Vp=20%','Vp=30%','Vp=40%','Vp=50%','Vp=60%','Vp=70%','Vp=80%','Vp=90%', 'R0=1');%,'Ef2') 
title(country);
